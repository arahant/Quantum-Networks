...
#
def get_route(matrix,start,end,file=False):
    try:
        ...
        path = []
        cost = []
        get_path(matrix,path,end,path,cost)
        print("Path from {} -> {}:".format(start,end),path)
        print("Cost from {} -> {}:".format(start,end),cost)
        if not file:
            file = "./route.csv"
        save_route(path,file)
        name_cost = "./cost.csv"
        save_route(cost,name_cost)
        return (path,cost,file)
    except SyntaxError:
        raise
...
def start():
    try:
        ...
        name_route = "./route.csv"
        save_route(path,name_route)
        name_cost = "./cost.csv"
        save_route(cost,name_cost)
